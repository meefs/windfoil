// windfoil-accel.wgsl — the windfoil coverage shader (src/windfoil.wgsl) with a MOMENTS + BACKDROP acceleration
// for minification, fused into one 2D-cell structure. Bit-for-bit identical coverage to windfoil; it only skips
// work the plain path would grind out. Kept in bench/ so src/windfoil.wgsl stays the thin reference. See
// docs/NOTES.md → "Band Moments" and "Backdrop"; buffers built by bench/windfoil-accel.js.
//
// windfoil is slowest at minification: a pixel's y-slab spans many row bands and it integrates every curve in
// each. For a band WHOLLY inside the slab we split its curves at a fixed grid of ACCEL_CELLS vertical cells
// (per-glyph, uniform over the ink box) and precompute per cell S = Σ∫(x−xref)·y′ and D = Σ∫y′. A fragment then
// classifies each cell against its pixel box:
//   • fully INSIDE → the clamp never clips → S + (xref − rc.x + hx)·D   (the moment, O(1))
//   • fully RIGHT  → the clamp saturates   → sx·D                        (the backdrop, O(1))
//   • fully LEFT   → 0
//   • straddling a box edge (≤ 2 cells)    → integrate that cell's split sub-pieces the real way.
// So an interior band of any curve count collapses to ~ACCEL_CELLS O(1) tests + ≤ 2 real integrations. Slab-EDGE
// bands (clipped y-window, where the per-band moment doesn't hold) fall back to windfoil's plain row gather.
...
// Per-cell fast-path data: (S, D, subStart, subCount). S = Σ∫(x−xref)·y′ and D = Σ∫y′ over the cell's split
// sub-pieces (xref = glyph bbox loX); [subStart, subCount] index `cellCurves` for the straddle integration.
// ACCEL_CELLS vec4 per band, indexed (rowBase + ri)·ACCEL_CELLS + cell. Built in bench/windfoil-accel.js.
@group(0) @binding(4) var<storage, read> cellData : array<vec4<f32>>;
// The split sub-pieces (3 vec2 each), grouped by cell — integrated only for the ≤2 cells a pixel box straddles.
@group(0) @binding(5) var<storage, read> cellCurves : array<vec2<f32>>;

const ACCEL_CELLS : u32 = 8u;   // vertical cells per glyph — MUST equal ACCEL_CELLS in windfoil-accel.js
...
// Same as integrate_band, but over a CELL's split sub-pieces in `cellCurves` — used for the ≤2 cells a pixel
// box straddles (its sub-pieces are already y-clipped to the band and confined to the cell in x).
fn integrate_cell(start : u32, count : u32, rc : vec2<f32>, wlo : f32, whi : f32, sx : f32) -> f32 {
  var acc : f32 = 0.0;
  let hx = sx * 0.5;
  for (var i : u32 = 0u; i < count; i = i + 1u) {
    let base = (start + i) * 3u;
    let q1 = cellCurves[base] - rc;
    let q2 = cellCurves[base + 1u] - rc;
    let q3 = cellCurves[base + 2u] - rc;
    if (max(q1.x, max(q2.x, q3.x)) <= -hx) { continue; }
    let lo = max(wlo, min(q1.y, q3.y));
    let hi = min(whi, max(q1.y, q3.y));
    if (hi <= lo) { continue; }
    if (min(q1.x, min(q2.x, q3.x)) >= hx) {
      acc += sx * (clamp(q3.y, wlo, whi) - clamp(q1.y, wlo, whi));
      continue;
    }
    acc += integrate_piece(q1, q2, q3, lo, hi, hx);
...
// One glyph's winding integral over the pixel box (rc ± s/2). The pixel's y-slab selects the band range; each
// band is read clipped to its y-range. windfoil-accel's twist: a band WHOLLY inside the slab is served by its
// 2D cells — S+..·D for cells fully inside the box, sx·D for cells fully right (backdrop), 0 for fully left,
// and only the ≤2 straddle cells integrate. Slab-edge bands fall back to the plain row-band gather.
fn integrate_face(I : Instance, rc : vec2<f32>, s : vec2<f32>) -> f32 {
  ...
  var ri0 : u32 = 0u;
  var ri1 : u32 = 0u;
  let banded = invH > 0.0 && R > 1u;   // the cell path only applies to multi-band glyphs
  if (banded) {
  ...
  // Cell grid: ACCEL_CELLS uniform cells over the glyph's x-extent (matches the build). xref = glyph loX.
  let xref = I.bbox.x;
  let cellW = (I.bbox.z - I.bbox.x) / f32(ACCEL_CELLS);
  let boxLo = rc.x - hx;   // absolute pixel-box x-range (rc is absolute em space)
  let boxHi = rc.x + hx;

  var f_int : f32 = 0.0;
  for (var ri = ri0; ri <= ri1; ri = ri + 1u) {
    var band_lo = -sy2;                               // rc-relative band y-range (== slab when unbanded)
    var band_hi = sy2;
    var w_lo = -sy2;                                  // rc-relative window edges — stable at any zoom
    var w_hi = sy2;
    if (banded) {
      band_lo = dy0 + f32(ri) / invH;
      band_hi = dy0 + (f32(ri) + 1.0) / invH;
      w_lo = max(w_lo, band_lo);
      w_hi = min(w_hi, band_hi);
    }
    if (w_hi <= w_lo) { continue; }

    let rIdx = (rowBase + ri) * 2u;
    let count = rows[rIdx + 1u];
    // Take the 2D-cell path only where it actually wins: an interior band (wholly inside the slab), DENSE
    // enough that the ~ACCEL_CELLS sweep beats integrating its curves, AND with the box wide enough to span
    // several cells (so many collapse to an O(1) moment/backdrop instead of an x-split straddle integration).
    // Everything else — sparse bands, slab-edge bands, magnified/medium boxes — takes windfoil's plain gather.
    if (banded && cellW > 0.0 && count > ACCEL_CELLS * 2u && s.x >= 3.0 * cellW && band_lo >= -sy2 && band_hi <= sy2) {
      let cellBase = (rowBase + ri) * ACCEL_CELLS;
      let wh = w_hi - w_lo;
      for (var cc : u32 = 0u; cc < ACCEL_CELLS; cc = cc + 1u) {
        let cellLo = xref + f32(cc) * cellW;
        let cellHi = cellLo + cellW;
        if (cellHi <= boxLo) { continue; }            // fully LEFT of the box → 0
        let cd = cellData[cellBase + cc];             // (S, D, subStart, subCount)
        if (cellLo >= boxHi) {                         // fully RIGHT → backdrop: full box width × net Δy
          f_int += s.x * cd.y;
          continue;
        }
        if (cellLo >= boxLo && cellHi <= boxHi) {     // fully INSIDE → moment (no clamp clipping)
          f_int += cd.x + (xref - rc.x + hx) * cd.y;
          continue;
        }
        f_int += integrate_cell(u32(cd.z), u32(cd.w), rc, w_lo, w_hi, s.x); // straddles a box edge → integrate
      }
      continue;
    }

    f_int += integrate_band(rows[rIdx], count, rc, w_lo, w_hi, s.x);
  }
  return f_int;
}
